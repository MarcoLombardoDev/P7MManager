...
#
# Start P7M Manager, after checking that the executable is the one this archive
# was built with.
#
...
if [ -d "$here/$EXE_NAME.app" ]; then
    exe="$here/$EXE_NAME.app/Contents/MacOS/$EXE_NAME"   # macOS: in the bundle
else
    exe="$here/$EXE_NAME"
fi

sums="$here/$EXE_NAME.sha256"
...
if [ "${P7MMANAGER_SKIP_VERIFY:-}" = "1" ]; then
    echo "$APP: checksum verification skipped (P7MMANAGER_SKIP_VERIFY=1)" >&2
elif [ ! -f "$sums" ]; then
    echo "$APP: $EXE_NAME.sha256 is missing, starting without checking" >&2
elif ! actual=$(digest_of "$exe"); then
    echo "$APP: no sha256 tool found, starting without checking" >&2
else
    # The file is in the format sha256sum -c reads: "<hex>  <path>". `read`
    # reports EOF on a file with no trailing newline but has assigned by then,
    # so its status is ignored and the emptiness checked instead.
    expected=""
    read -r expected _ < "$sums" || :
    if [ -z "$expected" ]; then
        echo "$APP: $EXE_NAME.sha256 is empty, starting without checking" >&2
    elif [ "$actual" != "$expected" ]; then
        echo "$APP: the executable does not match $EXE_NAME.sha256." >&2
